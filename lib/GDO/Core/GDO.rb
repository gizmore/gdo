...
#
# Table methods may require you to call the method on the GDO_YourFoo.table` object.
#
# You create tables by overriding the "fields" method which returns an array of GDT.
# GDT are plugged into GDO fields, or other GDT that support the WithFields decorator like GDT_Form, GDT_Table, Method parameters and more.
#
#
# @version 1.00
# @since 1.00
...
class GDO::Core::GDO
  ...
  #
  # Database columns.
  # This is only called by the caching db connection class.
  #
  ...
  def id; @id ||= _get_id; end
  def get_id; @id ||= _get_id; end
  def _get_id
    id = ""
  ...
  def self.blank(data={})
    ...
    instance.columns.each {|k,gdt|
      if data.has_key?(k.to_sym); var = data[k.to_sym]
      elsif data.has_key?(k.to_s); var = data[k.to_s]
      else; var = gdt._initial
      end
      var = var.to_s unless var.nil?
      instance.set_var(k, var, true)
    }
  ...
  def delete_where(where="1")
    deleted = 0
    result = table.select.where(where).execute
    while gdo = result.fetch_object
      gdo.delete
      deleted += 1
    end
    self
  ...
  def count_where(where="1")
    query.select('COUNT(*)').where(where).execute.fetch_var.to_i
  end
  ...
  ### Events ###
  ##############
  def before_create(query); columns.each{|name,gdt| gdt.before_create(self, query) }; end
